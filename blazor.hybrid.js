(function () {
    ...
    var loadScript = async function (name, callback) {
        ...
        let scriptPath = '_framework/blazor.' + name + '.js';
        const fileExists = file =>
            fetch(file, { method: 'HEAD', cache: 'no-store' })
                .then(response => response.status == 200);

        let yourFileExists = await fileExists(scriptPath);
        if (yourFileExists === true) {
